...
func revel_module(project string, template_dir string, name string, theme string, force bool) error {
	if err := revel_model(project, template_dir, name, force); err != nil {
		return err
	}

	if err := revel_controller(project, template_dir, name, force); err != nil {
		return err
	}

	if err := revel_view(project, template_dir, name, theme, force); err != nil {
		return err
	}
	return nil
}

func revel_model(project string, template_dir string, name string, force bool) error {
	gopath := build.Default.GOPATH
	if gopath == "" {
		return errors.New("Abort: GOPATH environment variable is not set. ")
	}
...
func revel_controller(project string, template_dir string, name string, force bool) error {
	gopath := build.Default.GOPATH
	if gopath == "" {
		return errors.New("Abort: GOPATH environment variable is not set. ")
	}
...
func revel_view(project string, template_dir string, name string, theme string, force bool) error {
	gopath := build.Default.GOPATH
	if gopath == "" {
		return errors.New("Abort: GOPATH environment variable is not set. ")
	}
